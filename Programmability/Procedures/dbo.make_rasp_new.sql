...
--2019-07-12 OD Убрана ссылка на vv03..w_all
...
CREATE PROCEDURE [dbo].[make_rasp_new]
  @id_job int,
  @N      int 
AS
BEGIN	
...
--declare @N as int =39995, @id_job as int =683332124
...
---------------ИНИЦИАЛИЗАЦИЯ ПАРАМЕТРОВ РАСПРЕДЕЛЕНИЯ--------
	...
	select @date_rasp=Date_r 
		...
		, @type_raspr = isnull([type_raspr],0)
...
-- значит обычное распределение  - нужно создать таблицы (tov_init , tov_kontr_init и тп)
	...
	if @test_raspr = 1 -- для распределения из тестовой базы данные готовятся другой хранимкой(данные из srv-sql05.fin_test_full)
		begin
			exec M2.dbo.prepare_test_raspred_1C @N , @id_job
		end
	else
		begin
			exec M2.dbo.prepare_raspred_1C @N , @id_job
		end
end
 ...
 begin
  ...
  where [Number_r]= @N
	...
	select  @dont_use_wait_sklad = Raspr_zadanie.dont_use_wait_sklad
			, @only_reserve = ISNULL(only_reserve,0)
	from M2..Raspr_zadanie   where Number_r=@N
...
and r.q_plan_pr>0
    ...
    BEGIN
      BEGIN try		
		...
		update M2..tov_kontr_init
		set q_ost_sklad = t.q_ost_sklad_calc
		from M2..tov_kontr_init t
		where t.Number_r= @N and t.q_ost_sklad_calc is not null -- значит было уже в prepare изменение q_ost_sklad
	...
	**/
    ...
    BEGIN TRY
		...
		select case when td.date_ost < {d'2010-01-01'} then @date_rasp 
			...
			, td.id_tov 
			, td.id_kontr 
			, max(case when td.date_ost_fact < {d'2010-01-01'} then @date_rasp 
					when td.date_ost_fact< {d'3016-01-01'} then td.date_ost_fact
					else DATEADD(year,-2000,td.date_ost_fact) end) date_ost_fact
...
select @id_job , 10030, DATEDIFF(MILLISECOND , @getdate ,GETDATE()),@test_raspr,SUBSTRING(@result_table,1,50) 
    ...
    BEGIN TRY
		...
		-- добавить новые
		insert into m2..tov_kontr_date (Number_r , id_tov , id_kontr , q_ost_sklad , date_ost, date_ost_fact)
		select @N , tdk.id_tov , tdk.id_kontr , tdk.q_ost_sklad , tdk.date_ost, tdk.date_ost_fact
...
where ttk.Number_r=@N
and ttk.tt_format_rasp in (2,4,12,14,18,400)


-- АК обнуления мин_остатков для срока годности менее 30 дней кроме Мягкого мороженого
update ttk
...
inner join m2..tt  tt1 on tt1.id_tt = ttk.id_tt
left join #MCK m on m.id_tt_mck=ttk.id_tt
where ttk.Number_r=@n and (tk.srok_godnosti< 30 
and t.id_group <> 10450  -- кроме мягкого мороженого
and t.id_group <> 10392  -- Отдел без упаковки (Сыпучие)
and t.id_group <> 10454  -- Айс ВкусВилл
and t.id_tov<>21810
and not (tt1.N=2376 and tov.id_group in (65, 10174, 10176)) -- убрать ФРОВ из 2376 магазина
) 
and (q_min_ost != 0 or min_ost_tt_tov!=0)
and m.id_tt_mck is null
and ttk.tt_format_rasp in (2,4,14,12,18,400)


	insert into jobs..Jobs_log ([id_job],[number_step],[duration]) 
	select @id_job , 200011, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) 
	select @getdate = getdate() 
...
tk.id_tov ,  isnull(sum(ttk.q_rashod_fact),0) q_rashod_fact, 
 case when isnull(sum(ttk.q_rashod_fact),0) < (sum(tk.q_ost_sklad)  + isnull(sum(ttk.q_rashod_fact),0)) * 0.5 -- добавить, что не было уже сбора более 50%
 then master.dbo.maxz(0,(((sum(tk.q_ost_sklad)  + isnull(sum(ttk.q_rashod_fact),0)) * 0.7) - isnull(sum(ttk.q_rashod_fact),0)) ) 
 else 0 end q_raspr
...
-- теперь найти характетиристики,  с которых собрать
	insert into jobs..Jobs_log ([id_job],[number_step],[duration]) 
	select @id_job , 200012, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) 
	select @getdate = getdate() 
...
SELECT  distinct t.id_tov 
  ...
  and ( SUM(tk.q_ost_sklad) < t.z_1 * 0.2)
	...
	insert into jobs..Jobs_log ([id_job],[number_step],[duration]) 
	select @id_job , 200013, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) 
	select @getdate = getdate() 
	
select distinct tr.id_tt,  isnull(tr.type_reserve,0) type_reserve
into #tt_type_reserve
from m2..tt_tov_kontr_init tr
where tr.Number_r = @N

--- Заказы покупаталей

if @sql_raspr + @test_raspr + @type_raspr = 0
begin

create table #q_zak (id_TT int, id_tov int, id_kontr int, BonusCard char(7) ,q real , ThisManufacturer int)
create table #q_zak_made ( id_TT int, id_tov int, id_kontr int, BonusCard char(7)  , ThisManufacturer int, q_raspr_cur real)

--declare @N int = 11660, @Date_Rasp date = '2019-06-10'
...
from sms_repl..td_move td  with (index(ind3))
...
--inner join M2..raspr_zadanie_tt ttz on ttz.id_tt = tt.id_TT and ttz.Number_r = @N

--left join #tov_only_VV t_vv on t_vv.id_tov = td.id_tov
inner join #tt_type_reserve tr on tr.id_tt = tt.id_TT
	
where td.operation_type in ( 802,803) and td.Confirm_type=1
and td.Date_proizv = dateadd(day,case when tr.type_reserve=1 or tt.id_region_tt = 61 then 1 else 0 end ,  @date_rasp ) 
...
-- удалить строки, которые в распределении с isnull(r.type_reserve,0) in (1,4)
...
select distinct r.id_tt
from m2..tt_tov_kontr_init r 
where r.number_r=@N and isnull(r.type_reserve,0) in (1,4) 
) r on r.id_tt = q.id_TT



	insert into jobs..Jobs_log ([id_job],[number_step],[duration]) 
	select @id_job , 200014, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) 
	select @getdate = getdate() 
	
-- найти строки, по которым было распределение, но сейчас не показывает q_rashod_fact (например, отменены расходники) -- считаем, что не было распределения
--declare @N int = 10515, @Date_Rasp date = '2019-06-02'

--declare @N int = 11660, @Date_Rasp date = '2019-06-10'

create table #raspzak_del (id_tt int, id_tov_pvz_rasp int )

insert into #raspzak_del
select tti.id_tt , tti.id_tov_pvz_rasp 
from 
(Select rz2.Number_r , tti.id_tt  , tti.id_tov_pvz_rasp , tti.q_rashod_fact
from M2..tt_tov_kontr_init tti with (index(ind3))
inner join m2..Raspr_zadanie rz on  rz.number_r = @N and rz.Date_r = @Date_Rasp 
inner join m2..Raspr_zadanie rz2 on  rz2.Date_r = @Date_Rasp and
rz2.number_r<@N and rz2.number_r> 0 and rz2.canceled is null and rz2.test_raspr=0 and rz2.date_reg_1C is not null and rz2.sql_raspr is null 
and  rz.id_sklad = rz2.id_sklad and rz.id_zone = rz2.id_zone
where tti.Number_r=@N --and tti.q_rashod_fact>0
) tti
--inner join M2..Tovari t on t.id_tov = tti.id_tov_pvz_rasp
--inner join M2..Tt tt on tt.id_tt = tti.id_tt

left join M2..archive_rasp r with (index(ind2)) on r.number_r = tti.Number_r and r.id_tt = tti.id_tt and r.id_tov_pvz_rasp = tti.id_tov_pvz_rasp
group by tti.id_tt , tti.id_tov_pvz_rasp , tti.q_rashod_fact
having  SUM(r.q_raspr)>0  and tti.q_rashod_fact=0

	insert into jobs..Jobs_log ([id_job],[number_step],[duration]) 
	select @id_job , 200015, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) 
	select @getdate = getdate() 
	

--drop table #q_zak_made


--  что уже распределено 

--declare @N int = 11730, @Date_Rasp date = '2019-06-10'
...
-- пересчитать в вес товара из заказа - q_raspr_cur в колве id_tov_new
select rz.id_tt , rz.id_tov  , rz.id_kontr  , rz.BonusCard , rz.ThisManufacturer  ,sum( case when ra.q_raspr>0 then  
rz.q_raspr_cur * case when tov.Ves <> tov2.Ves and (tov.Ves=1 or tov2.ves =1) then tov2.Ves/tov.ves  else 1 end else 0 end  ) q_raspr_cur
--*
from m2..rasp_zakaz_pok rz
inner join m2..Raspr_zadanie r on rz.Number_r = r.number_r
inner join M2..archive_rasp ra on ra.number_r = r.Number_r and ra.id_tt = rz.id_tt and ra.id_tov = ISNULL(rz.id_tov_new,rz.id_tov)
and (ra.id_kontr = ISNULL(rz.id_kontr_new,rz.id_kontr) or rz.ThisManufacturer=0) and rz.q_raspr>0

inner join M2..tov_init tov on tov.id_tov = rz.id_tov and tov.Number_r = @N
inner join M2..tov_init tov2 on tov2.id_tov = ISNULL(rz.id_tov_new,rz.id_tov) and tov2.Number_r = @N

inner join M2..raspr_zadanie_tt ttz on ttz.id_tt = rz.id_TT and ttz.Number_r = @N

inner join M2..Raspr_zadanie (nolock) rz1 on rz1.id_sklad = r.id_sklad and rz1.id_zone = r.id_zone and rz1.Number_r = @N -- чтоб совпал склад

left join #raspzak_del rd on rd.id_tt = rz.id_tt and rd.id_tov_pvz_rasp = rz.id_tov_pvz -- если есть q_rashod_fact то не считать, что уже рспределили

where rd.id_tt is null and r.Date_r = @Date_Rasp and rz.q_raspr_cur>0
and r.number_r<@N and r.number_r> 0 and r.canceled is null and r.test_raspr=0 and r.date_reg_1C is not null and r.sql_raspr is null
--and rz.id_tt = 12618


group by rz.id_tt , rz.id_tov  , rz.id_kontr  , rz.BonusCard , rz.ThisManufacturer



	insert into jobs..Jobs_log ([id_job],[number_step],[duration]) 
	select @id_job , 200016, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) 
	select @getdate = getdate() 
	
	
--declare @N int = 10332, @Date_Rasp date = '2019-06-01'
insert into m2..rasp_zakaz_pok
 ...
 --select * from #q_zak_made    
      
select @N Number_r , td.id_TT , td.id_tov , td.id_kontr , td.BonusCard ,  
case when tk.Kolvo_korob is not null then  master.dbo.minz(td.q - ISNULL(qz.q_raspr_cur,0) - (k.Korob-1)*tk.Kolvo_korob,tk.Kolvo_korob) else td.q - ISNULL(qz.q_raspr_cur,0) end q, 
td.ThisManufacturer, 0 q_raspr , Null id_kontr_new, 0, 
case when tk.Kolvo_korob is not null then tk.Kolvo_korob else td.q - ISNULL(qz.q_raspr_cur,0) end Kolvo_korob , k.Korob , ISNULL(tov.id_tov_pvz , tov.id_tov) , 0
from #q_zak td
...
where 
(
((k.Korob-1)*tk.Kolvo_korob < td.q - ISNULL(qz.q_raspr_cur,0) +0.1 and  ((td.q - ISNULL(qz.q_raspr_cur,0) - (k.Korob-1)*tk.Kolvo_korob)>tk.Kolvo_korob*0.1 or k.Korob=1) and tk.Kolvo_korob is not null) 
or (k.Korob=1 and tk.Kolvo_korob is null ) 
)
and ( td.q>qz.q_raspr_cur or qz.q_raspr_cur is null)
and (td.id_tov not in (30630, 30631, 30632, 30633, 30634, 30574, 30576) or (k.Korob=1 and qz.q_raspr_cur=0)) -- Для весовой кулинарии ЗиЗ - только одну коробку на один заказ
order by td.id_TT , td.id_tov , td.id_kontr , td.BonusCard, td.ThisManufacturer , k.Korob 


	insert into jobs..Jobs_log ([id_job],[number_step],[duration]) 
	select @id_job , 200017, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) 
	select @getdate = getdate() 
...
left join #q_zak qz on qz.id_TT = ttk.id_tt and qz.id_tov = ISNULL(ti.id_tov_pvz , ti.id_tov )
where ttk.Number_r = @N and qz.id_TT is  null



-- если не все магазины - вычесть с остатков заказы покупателям невыполненные по непопавшим магазинам

---declare @N int = 21650 , @date_rasp date = '2019-08-21'
select distinct t1.id_tt 
into #tt_nepopali
from m2..Raspr_zadanie rz
inner join m2..Raspr_zadanie rz2 on rz2.Date_r = rz.Date_r and rz2.id_zone = rz.id_zone
inner join m2..raspr_zadanie_tt t1 on t1.number_r = rz2.Number_r
left join m2..raspr_zadanie_tt t2 on t1.id_tt = t2.id_TT and t2.number_r = @N
where rz.Number_r = @N and t2.id_tt is null

--return


if exists (select * from #tt_nepopali)
begin



create table #q_zak_nepopali (id_TT int, id_tov int, id_kontr int, BonusCard char(7) ,q real , ThisManufacturer int)
create table #q_zak_made_nepopali ( id_TT int, id_tov int, id_kontr int, BonusCard char(7)  , ThisManufacturer int, q_raspr_cur real)

--declare @N int = 21650, @Date_Rasp date = '2019-08-21'

insert into #q_zak_nepopali
...
inner join #tt_nepopali t on t.id_tt = tt.id_TT 
inner join M2..tov_init tov on tov.id_tov = td.id_tov and tov.Number_r = @N
--inner join #tt_nepopali ttz on ttz.id_tt = tt.id_TT 
...
insert into #q_zak_made_nepopali
...
inner join #tt_nepopali ttz on ttz.id_tt = rz.id_TT 

inner join M2..Raspr_zadanie (nolock) rz1 on rz1.id_sklad = r.id_sklad and rz1.id_zone = r.id_zone and rz1.Number_r = @N -- чтоб совпал склад


where  r.Date_r = @Date_Rasp and rz.q_raspr_cur>0
and r.number_r<@N and r.number_r> 0 and r.canceled is null and r.test_raspr=0 and r.date_reg_1C is not null and r.sql_raspr is null
--and rz.id_tt = 12618
...
--declare @N int = 21650, @Date_Rasp date = '2019-08-21'
update m2..tov_kontr
set q_ost_sklad = master.dbo.maxz(0,tk.q_ost_sklad - a.q) , q_ost_sklad_calc = master.dbo.maxz(0,tk.q_ost_sklad_calc - a.q)
from m2..tov_kontr tk
inner join 
(select q1.id_tov , q1.id_kontr , q1.ThisManufacturer , sum(q1.q - isnull(q2.q_raspr_cur,0)) q
from #q_zak_nepopali q1
left join #q_zak_made_nepopali q2 on q1.id_tov = q2.id_tov and q1.id_TT = q2.id_TT 
and q1.id_kontr = q2.id_kontr and q1.BonusCard = q2.BonusCard and q1.ThisManufacturer = q2.ThisManufacturer
group by q1.id_tov , q1.id_kontr , q1.ThisManufacturer
) a on a.id_tov = tk.id_tov and a.id_kontr = tk.id_kontr
where a.ThisManufacturer=1 and tk.Number_r=@N
...
inner join 
(
select v.id_tov , v.id_kontr_new id_kontr, sum(v.q) q
from
(
select top 1 with ties a.id_tov , a.id_kontr , a.q , tk.id_kontr id_kontr_new
from
(select q1.id_tov , q1.id_kontr , q1.ThisManufacturer , sum(q1.q - isnull(q2.q_raspr_cur,0)) q
from #q_zak_nepopali q1
left join #q_zak_made_nepopali q2 on q1.id_tov = q2.id_tov and q1.id_TT = q2.id_TT 
...
inner join m2..tov_kontr tk on a.id_tov = tk.id_tov and tk.Number_r=@N
where a.ThisManufacturer=0 and tk.q_ost_sklad >=a.q
order by ROW_NUMBER() over (partition by a.id_tov , a.id_kontr order by tk.q_ost_sklad desc)
) v
group by v.id_tov , v.id_kontr_new 
) a on a.id_tov = tk.id_tov and a.id_kontr = tk.id_kontr
where tk.Number_r=@N
...
--and t.raspr_double=1 --and t.raspr_d_1_2=2
	...
	insert into jobs..Jobs_log ([id_job],[number_step],[duration]) 
	select @id_job , 200018, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) 
	select @getdate = getdate() 
	...
	insert into jobs..Jobs_log ([id_job],[number_step],[duration]) 
	select @id_job , 200019, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) 
	select @getdate = getdate() 
...
**/
	...
	--IF OBJECT_ID('tempdb..#DocPeredanoNaTT') IS NOT NULL
	--DROP TABLE #DocPeredanoNaTT

	--CREATE TABLE #DocPeredanoNaTT (
	...
	--INSERT INTO #DocPeredanoNaTT
	--SELECT CAST(master.dbo.Binary2UID(dv._Document3034_IDRRef) AS NVARCHAR(36))
	--	,convert(DATE, date_1c)
	--	,id_tt_1c
	--	,id_tov_1c
	--	,_Fld3105
	--FROM M2.dbo._Document3034_VT3100_vw dv(NOLOCK)
	--INNER JOIN IzbenkaFin.._Reference29 ref ON dv._Fld3102RRef = ref._IDRRef
	--WHERE dv._Posted_1c = 1
	--	AND dv._Fld3096RRef_1C = 0xAE2C7DEE08A154D2489A2C17DCB24F70
	--	AND dv._Fld3088RRef_1C = 0xBE738D5D0F666EE64FC467BA53367442
	--	AND dv.date_1c >= CONVERT(DATE, DATEADD(DAY, - 1, @date_rasp))
	--	AND ref._Fld5908 = 0x01

	--CREATE CLUSTERED INDEX docIdIndex ON #DocPeredanoNaTT (id_doc)
	...
	--		SELECT zht.id_tt
	--			,zht.id_tov
	--			,zht.колво - master.dbo.maxz(0, ISNULL(f.q, 0)) колво
	...
	--				,sum(q) q
	--			FROM (
	--				SELECT CONVERT(DATE, td.closedate) date_f
	--						,tt.id_TT
	--						,td.id_tov
	--						,SUM(td.Quantity * t_o.znak) q
	--					FROM SMS_REPL.dbo.TD_move AS td
	--					INNER JOIN M2..tt ON tt.N = td.ShopNo_rep
	--					INNER JOIN SMS_REPL.dbo.Types_Operation AS t_o ON td.operation_type = t_o.code_operation
	--						AND t_o.table_operation = 'Td_move'
	--						AND t_o.field_operation = 'operation_type_orig'
	--					WHERE (
	--							td.operation_type IN (
	--								400
	--								,401
	--								)
	--							)
	...
	--					GROUP BY CONVERT(DATE, td.closedate)
	--						,tt.id_TT
	--						,td.id_tov

	--					union all

	--					SELECT date_1C as date_f
	--						,dv.id_tt
	--						,dv.id_tov
	--						,dv.q
	--					FROM #DocPeredanoNaTT dv(NOLOCK)
	--					WHERE dv.id_doc not in (select distinct Id_doc from  SMS_REPL.dbo.TD_move where NOT (operation_type IN (400,401) AND ISNULL(Confirm_type, 0) IN (1)))
	--				) s
	--				group by date_f, id_tt, id_tov
	--			) f ON f.date_f = dateadd(day, 1, zht.date_ch)
	--			AND f.id_tov = zht.id_tov
	--			AND f.id_TT = zht.id_tt
	...
	--		SELECT zht.id_tt
	--			,zht.id_tov
	--			,zht.колво - master.dbo.maxz(0, ISNULL(f.q, 0))
	--		FROM M2..Zakaz_Hoz_Tovari zht
	--		LEFT JOIN (
	--					SELECT date_f
	--				,id_tt
	...
	--	update m2..tt_tov_kontr
	--		set q_plan_pr = b.колво * b.Kolvo_korob , koef_tt=0 , q_min_ost = 0 , q_FO=0 , id_kontr = b.id_kontr
	--	from m2..tt_tov_kontr ttk with (index (ind1))
	--	inner join 
	--		(
	--			select top 1 with ties ttk.id_tt , ttk.id_tov , ttk.id_kontr , zht.колво , tk.Kolvo_korob
	--			from m2..tt_tov_kontr ttk with (index (ind1))
	--				inner join M2..tov_kontr tk on ttk.id_tov=tk.id_tov and tk.Number_r=@N
	--				inner join #zht zht on zht.id_tov=ttk.id_tov and zht.id_tt=ttk.id_tt
	--			where ttk.Number_r=@N
	--			order by ROW_NUMBER() over (partition by ttk.id_tt , ttk.id_tov order by tk.q_ost_sklad desc)
	--		) b on ttk.id_tt = b.id_tt and ttk.id_tov = b.id_tov
	--	where ttk.Number_r=@N
	--end
	...
	begin
		...
		SELECT id_tt
			...
			,SUM(q) q
		into #tov_v_puti
		FROM #DocPeredanoNaTT (NOLOCK)
		WHERE id_doc not in (select distinct Id_doc from  SMS_REPL.dbo.TD_move where operation_type IN (400,401) AND ISNULL(Confirm_type, 0) = 1)
			  and date_1c = CONVERT(DATE, DATEADD(DAY, - 1, @date_rasp))
		group by id_tt,id_tov

		SELECT top 1 with ties date_ch, id_TT, id_tov, колво
		INTO #actual_zht
		FROM M2..Zakaz_Hoz_Tovari zht
		order by ROW_NUMBER() over (partition by id_TT, id_tov order by date_ch desc) 

		INSERT INTO #zht
		SELECT zht.id_TT, zht.id_tov, sum(zht.колво - isnull(p.q, 0) - isnull(tvp.q, 0)) 
		FROM #actual_zht zht
		LEFT JOIN
			(SELECT CONVERT(DATE, td.closedate) date_f
					,tt.id_TT
			...
			WHERE  td.operation_type IN (400,401)
				AND ISNULL(td.Confirm_type, 0) IN (1)
				AND td.closedate >= CONVERT(DATE, DATEADD(DAY, - 1, GETDATE()))
			GROUP BY CONVERT(DATE, td.closedate)
				,tt.id_TT
				,td.id_tov) p ON p.date_f > zht.date_ch 
							 AND p.id_tov = zht.id_tov 
							 AND p.id_TT = zht.id_tt
		LEFT JOIN #tov_v_puti tvp on zht.id_tov = tvp.id_tov and zht.id_tt = tvp.id_tt
		group by  zht.id_TT, zht.id_tov
		having sum(zht.колво - isnull(p.q, 0)- isnull(tvp.q, 0)) > 0
...
**/
	...
	and ttk.tt_format_rasp in (2,4,12,400)
   ...
   (
	...
	(select distinct ttk.id_tov , ttk.id_tt from  #ttk_tt_tov_0 ttk ) ttk on v.id_tov = ttk.id_tov and v.id_tt = ttk.id_tt 
      ...
      begin
		  ...
		  insert into #w_all_v(date_r,id_tt,id_tov,Fact,q,rn,date_add,rn_2,Date_f)
		  select v.date_r,v.id_tt,v.id_tov,v.Fact,v.q,v.rn,v.date_add,v.rn_2,v.Date_f 
		  from  m2..w_all_01   v
			  inner join (select distinct ttk.id_tov , ttk.id_tt 
							from  #ttk_tt_tov_0 ttk) ttk
				 on v.id_tov = ttk.id_tov 
					and v.id_tt = ttk.id_tt 
			        and v.date_r = @date_rasp
...
**/
  ...
  create unique clustered index ind1 on #rasp_for_sales (id_tt , id_tov )
   ...
   ------------------------------------------------------------------------------------------
	...
	and a.tt_format not in (4,10,12,14,16)
	...
	create table #rasp_1 (id_tt int , id_tov int, Fact real )
   ...
   insert into [M2].[dbo].[rasp_smena_kontr]
	...
	--select * from #add_r2_2

	-- если не нужна характеристика, но есть осткатки - перенести на самую с большим планом - по каждой характеристике найти одну тт

	select r.id_tov , b.id_kontr , r.id_kontr id_kontr_2 , r.id_kontr_r , c.id_tt
    into #add_r2_2
	from
	(select tk.id_tov , tk.id_kontr ,  r.id_kontr id_kontr_r  ,
	ROW_NUMBER() over (partition by tk.id_tov order by  tk.id_kontr) rn
	 from M2..tov_kontr tk with (  INDEX(IX_tov_kontr_1) ) 
	 left join
	  (
	  select r.*
	  ...
	  having count(*)=1
	  ) f on f.id_tt = r.id_tt and f.id_tov = r.id_tov
	  where r.Number_r = @N and r.tt_format_rasp <> 10
	  ) r on tk.id_tov=r.id_tov and tk.id_kontr=r.id_kontr

    where tk.q_ost_sklad>0 and tk.Number_r = @N
	group by tk.id_tov , tk.id_kontr, r.id_kontr 
	having isnull(sum(r.q_plan_pr),0) = 0 
	) r
	inner join
	(	
	Select   r.id_tov , r.id_kontr ,
	ROW_NUMBER() over (partition by r.id_tov  order by count(id_tt)  desc) rn_b
	 from M2..tt_tov_kontr r with ( index (ind1))
	 inner join M2..tov_kontr tk with (  INDEX(IX_tov_kontr_1) ) on tk.Number_r=@N and tk.id_tov=r.id_tov and tk.id_kontr=r.id_kontr
	where r.number_r = @N and tk.q_ost_sklad_calc>0
	group by r.id_tov , r.id_kontr
	having sum(r.q_plan_pr)>0
	) b on r.id_tov = b.id_tov and b.rn_b=1
	inner join
	(Select  r.id_tov , r.id_kontr , r.id_tt , r.q_plan_pr, 
	 ROW_NUMBER() over (partition by r.id_tov , r.id_kontr order by r.q_FO -r.q_plan_pr ) rn
	 from M2..tt_tov_kontr r with ( index (ind1))
	where r.number_r = @N and r.q_plan_pr>0
		and r.tt_format_rasp in (2,400)
	) c on c.id_tov = b.id_tov and c.id_kontr = b.id_kontr and c.rn = r.rn
	where @dont_use_wait_sklad=0
	...
	from M2..tt_tov_kontr r with (index (ind1))
	inner join #add_r2_2 b on r.id_tov = b.id_tov  and r.id_tt = b.id_tt 
	where r.number_r = @N 
   ...
   insert into [M2].[dbo].[rasp_smena_kontr]
      ...
      ,[id_kontr_init]
      ,[type_smena])
    Select @N , id_tt , id_tov ,  id_kontr_2 ,id_kontr , 56 
    from #add_r2_2 
	...
	* case when ISNULL(tov.koef_tov,0)<=5 then (1+tov.koef_tov) else 1 end ,ttk.q_min_ost )
	...
	inner join m2..tov_kontr tk with (  INDEX(PK_tov_kontr) ) on ttk.id_tov=tk.id_tov and ttk.id_kontr=tk.id_kontr and tk.Number_r=@N
	--inner join M2..tt on tt.id_TT = ttk.id_tt and tt.tt_format not in (7,10)
	where ttk.Number_r=@N and tov.koef_tov>-1 and tk.rasp_all=1 and  tov.koef_tov<>0
    and ttk.tt_format_rasp not in (7,10)
	--------------------------------------------------------------------------------------------------
...
**/
    ...
    when @id_zone in (338,4550) then 0.2 else 0.5 end  *  master.dbo.maxz( r.Fact, ttk.q_plan_pr )  ) 
	 ...
	 0 [sort_ost] , 0 [sort_pr] , @N [number_r], ttk.id_kontr_init  [id_kontr_init], ttk.id_zal , ttk.q_zakaz , ttk.tt_format_rasp ,ttk.price_rasp, ttk.koef_ost_pr_rasp , ttk.id_tov_pvz_rasp
	 , ttk.q_plan_pr_2 ,  ttk.type_reserve , ttk.q_reserve_fact q_fo_2--, ttk.FreshForShipment
	 into #t_rastr_tmp
	 FROM M2..tt_tov_kontr ttk with ( index (ind1)) 
	 inner join m2..tov_kontr tk with (  INDEX(PK_tov_kontr) ) on ttk.id_tov=tk.id_tov and ttk.id_kontr=tk.id_kontr and tk.Number_r=@N
	 inner join #rasp_1 r on ttk.id_tov=r.id_tov and ttk.id_tt=r.id_tt
	...
	BEGIN TRY 
      ...
      ttk.q_plan_pr_2   , ttk.type_reserve  , ttk.q_reserve_fact q_fo_2--, ttk.FreshForShipment
		 ...
		 and tk.rasp_all=1
		 and isnull(case when isnull(rs.q_max,0)>0 then 100* rs.Fact / rs.q_max end,0) <1000000
        BREAK
      END TRY
      BEGIN CATCH
        IF ERROR_NUMBER() = 1205 -- вызвала взаимоблокировку ресурсов                          
...
create table #rasp_mck (id_tt int, id_tov int ,q_max_ost real, q_FO real , id_kontr int , q_raspr real , q_plan_pr real , srok_godnosti int , q_min_ost real, id_kontr_init int)
...
create table #nuzno_mck (id_tov int , q_nuzno real, q_nuzno_fact real null, id_kontr int null , id_kontr_rasp int null , id_kontr_init int)
...
INNER JOIN (
	...
	--			,a.i
	--			,ROW_NUMBER() OVER (
	--				PARTITION BY r.id_tov ORDER BY a.i
	--					,r.q_fo
	--				) rn
	--		FROM #rasp_mck r
	--		INNER JOIN (
	...
	--		WHERE CASE 
	--				WHEN r.q_max_ost > 0
	--					THEN master.dbo.maxz(0, master.dbo.minz(r.q_plan_pr + master.dbo.maxz(r.q_min_ost, r.q_plan_pr * CASE 
	--										WHEN srok_godnosti > 9
	--											THEN 1
	--										ELSE 0
	--										END) - r.q_FO, r.q_max_ost - r.q_FO))
	--				ELSE master.dbo.maxz(0, r.q_plan_pr + master.dbo.maxz(r.q_min_ost, r.q_plan_pr * CASE 
	--								WHEN srok_godnosti > 9
	--									THEN 1
	--								ELSE 0
	--								END) - r.q_FO)
	--				END > 0
	--		) s
	--	INNER JOIN #nuzno_mck n ON n.id_tov = s.id_tov
	--		AND s.rn <= n.q_nuzno_fact - n.q_nuzno
			...
			,CASE 
				WHEN r.q_max_ost > 0
					THEN master.dbo.maxz(0, master.dbo.minz(r.q_plan_pr + master.dbo.maxz(r.q_min_ost, r.q_plan_pr * CASE 
										...
										ELSE 0
										END) - r.q_FO, r.q_max_ost - r.q_FO))
				ELSE master.dbo.maxz(0, CEILING(r.q_plan_pr) - r.q_FO)
				END q
		FROM #rasp_mck r
		WHERE CASE 
				WHEN r.q_max_ost > 0
	...
	GROUP BY f.id_tt
		,f.id_tov
	) g ON g.id_tt = r.id_tt
	AND g.id_tov = r.id_tov
...
----------------------------------------------------------------------------------------------------------------------------------
   ...
   create table #ttk2 (id_tt int , id_tov int , q_new real)
	...
	select ttk.id_tt , ttk.id_tov , master.dbo.maxz(master.dbo.minz(r.Fact*(1+ISNULL(ttk.koef_pov_naib,0)*0.01),ttk.q_plan_pr)
	* case when ISNULL(tov.koef_tov,0)<=5 then (1+tov.koef_tov) else 1 end --+ ISNULL(ttk.q_zakaz,0) 
	...
	and q_plan_pr <> master.dbo.maxz(master.dbo.minz(r.Fact*(1+ISNULL(ttk.koef_pov_naib,0)*0.01),ttk.q_plan_pr)
	* case when ISNULL(tov.koef_tov,0)<=5 then (1+tov.koef_tov) else 1 end ,ttk.q_min_ost )
	and ttk.tt_format_rasp not in (10)
	and r.Fact>0
...
select @j1 = @j1+1 
   ...
   insert into [M2].[dbo].[rasp_smena_kontr]
	...
	-- -1 проход - закрываем 0 и меньше 0 по остаткам -- убрать товары, с нехваткой боле 20%
     ...
     -- добавляем формирование резерва, если в rasp  type_reserve=1,3
     -- делаем прямо в rasp,
     
     
     --q_fo_2 рассчитываем как норматив q_plan_pr * коэф остатка +  что уже распределено до этого в резерв q_reserve_fact
     --q_raspr_2 = заказы покупателей_2 дня_вперед  +   q_plan_pr_2 * коэф остатка - q_fo_2 
      
      --declare @N int = 15094
      
      --select * from #type_reserve_1
      --select * from #tov_type_reserve_1
      
      create table #type_reserve_1 (id_tt int , id_tov int , q_fo_2 real ,q_raspr_2 real )
      
      insert into #type_reserve_1
      select r.id_tt , r.id_tov  ,
		q_fo_2 =  master.dbo.maxz(r.q_ko_ost, r.q_plan_pr * r.koef_ost_pr_rasp * 0.01),
		q_raspr_2 = master.dbo.maxz(0,r.q_plan_pr_2 * ( 1 +  r.koef_ost_pr_rasp * 0.01 ) - master.dbo.maxz(r.q_ko_ost - r.q_plan_pr, r.q_plan_pr * r.koef_ost_pr_rasp * 0.01)) -- - r.q_fo_2
      from M2..rasp r 
		  inner join m2..Raspr_zadanie rz on rz.Number_r = r.number_r
		  inner join m2..Tov t on t.Number_r = r.number_r and t.id_tov = r.id_tov and t.id_tov_pvz is null -- пока кроме полных аналогов
		inner join m2..tt on tt.id_TT = r.id_tt
	  where r.number_r=@N and r.type_reserve in (1,3)
      
      and (rz.id_zone in ( 30 , 458 )  or (rz.id_sklad =8 and tt.id_region_tt  =61))
      
	  
	  --and id_region_tt <> 82


     -- найти id_kontr, которого хватает и с самым большим сроком годности
     create table #tov_type_reserve_1 ( id_tov int , id_konnt int ,q_raspr_2 real )
     
     insert into #tov_type_reserve_1
        -- declare @N int =  14920      
     Select top 1 with ties r.id_tov , tk.id_kontr ,ceiling(r.q_raspr_2/tk.Kolvo_korob) * tk.Kolvo_korob q_raspr_2
     from 
     (select  r.id_tov , SUM(r.q_raspr_2) q_raspr_2
     from #type_reserve_1 r 
     group by r.id_tov )r
     inner join M2..tov_kontr tk on tk.Number_r = @N and tk.id_tov = r.id_tov
     where ceiling(r.q_raspr_2/tk.Kolvo_korob) * tk.Kolvo_korob <=tk.q_ost_sklad and tk.q_wait_sklad=0
     and tk.srok_godnosti>=4
     order by ROW_NUMBER() over (partition by r.id_tov order by tk.srok_godnosti desc , tk.q_ost_sklad desc)
     
     
     -- найти разницу в неполной коробке и добавить на самый большой магаизин в #type_reserve_1
     
     update #type_reserve_1
     set q_raspr_2 = r0.q_raspr_2 + a.q_d
     from #type_reserve_1 r0   
     inner join
     (Select top 1 with ties r.id_tov, r.id_tt , r.q_raspr_2
     from #type_reserve_1 r
     order by ROW_NUMBER() over (partition by r.id_tov order by r.q_raspr_2 desc)
     ) r on r.id_tov = r0.id_tov and r.id_tt = r0.id_tt
     inner join 
     (select r1.id_tov , r1.q_raspr_2 - r.q_raspr_2 q_d
     from #tov_type_reserve_1 r1
     inner join
     (select  r.id_tov , SUM(r.q_raspr_2) q_raspr_2
     from #type_reserve_1 r 
     group by r.id_tov ) r on r.id_tov = r1.id_tov
     where ABS(r1.q_raspr_2 - r.q_raspr_2)>0.01
     ) a on a.id_tov = r.id_tov
     ...
     -- обновить rasp
       ...
       set
       --select 
       id_kontr = case when ttr.id_konnt is not null then ttr.id_konnt else r.id_kontr end ,
       q_raspr_2 = case when ttr.id_konnt is not null then tr.q_raspr_2 else 0 end,
       q_fo_2 = case when ttr.id_konnt is not null then tr.q_fo_2 else r.q_fo_2 end
       
       from  m2..rasp r
       inner join #type_reserve_1 tr on r.id_tt = tr.id_tt and r.id_tov = tr.id_tov
       left join  #tov_type_reserve_1 ttr on ttr.id_tov = r.id_tov
       where r.number_r = @N
           
      
     -- убрать с остатков в tov_kontr
      --declare @N int = 15094   
      update m2..tov_kontr
      set q_ost_sklad = tk.q_ost_sklad - ttr.q_raspr_2 , q_ost_sklad_calc = tk.q_ost_sklad_calc - ttr.q_raspr_2
      from m2..tov_kontr tk 
      inner join  #tov_type_reserve_1 ttr on ttr.id_tov = tk.id_tov  and ttr.id_konnt = tk.id_kontr
      where tk.Number_r= @N   
	...
	**/
	...
	-- первое распределение товара - rasp_all=0 - q_nuzno, rasp_all=1 q_ost_sklad_calc пропорционально q_nuzno - 1 коробка
	 ...
	 having SUM (ceiling( (master.dbo.maxz(0,rasp.q_nuzno - rasp.q_raspr)) / tk.Kolvo_korob ) * tk.Kolvo_korob) >0
		 ...
		 -- выявить товары, что нужно запустить в алгоритм распределения по срокам
	 -- есть одновременно разные характеристики, у который более половины сроки и менее половины
	 --declare @date_rasp date = '2019-08-22'
	 
	 select distinct a.id_tov
	 into #tov_new_algor
	 from 
	 (select  tkd.id_tov , tkd.id_kontr
	 --sum( case when datediff(day, tkd.date_ost , @date_rasp) > tk.srok_godnosti * 0.5 then tkd.q_ost_sklad end ),
	 --sum( case when datediff(day, tkd.date_ost , @date_rasp) <= tk.srok_godnosti * 0.5 then tkd.q_ost_sklad end )
	 from m2..tov_kontr_date tkd
	 inner join m2..tov_kontr tk on tk.Number_r = tkd.Number_r and tk.id_tov = tkd.id_tov and tk.id_kontr = tkd.id_kontr and tk.rasp_all=0
	 where tkd.Number_r = @N
	 group by tkd.Number_r , tkd.id_tov , tkd.id_kontr
	 having
	 isnull(sum( case when datediff(day, tkd.date_ost , @date_rasp) > tk.srok_godnosti * 0.5 then tkd.q_ost_sklad end ),0)>0
	 and isnull(sum( case when datediff(day, tkd.date_ost , @date_rasp) <= tk.srok_godnosti * 0.5 then tkd.q_ost_sklad end ),0)>0
	 )a
	 ...
	 left join #tov_new_algor tng on tng.id_tov = rasp.id_tov
	 ...
	 and rasp.tt_format_rasp in (2,10,400) ) or tk.rasp_all=0 )

     and @sql_raspr = 0 and @only_zakaz=0
     
     and not (isnull(rasp.type_reserve,0) in (1,4)) -- значит это добавлены тт, которые уже отправлены
	 	 
	 --and tk.rasp_all = 1 -- это осталось только для распределения полностью распределяемых товаров
	 
	 and tng.id_tov is null


	 	 update #rasp
 
 set 
 znach = case ttr.ТипОкруг when 1 then 
 ceiling((ra.q_min_ost) / tk.Kolvo_korob - 0.01) * tk.Kolvo_korob 
 else
 floor((ra.q_min_ost) / tk.Kolvo_korob+0.01) * tk.Kolvo_korob 
 end 

	 from  #rasp r
	  inner join m2..rasp ra on ra.id_tov=r.id_tov and ra.id_tt=r.id_tt and ra.Number_r=@N
	  inner join M2..tov_kontr tk with (INDEX(PK_tov_kontr)) on tk.id_tov=r.id_tov and tk.id_kontr=r.id_kontr and tk.Number_r=@N
	  left join m2..tt_Type_Rounding ttr on ttr.id_tt = r.id_tt
	 where r.tt_format_rasp = 10
...
-- новый алгоритм распределления складируемых товаров


/**
1.
проверять , что в  tov_kontr_date не меньше, чем в tov_kontr по id_kontr
разницу убрать у самых свежих tov_kontr_date

сортируем по дате   id_tov , id_kontr , date , суммируем нарастающим из tov_kontr_date
значения, где нарастающее меньше S ( id_tov , id_kontr from tov_kontr), оставляем равным min(Знач,S-Нараст)
Знач Нараст
а         0
b        a
c        a+b

И оставшуюся разницу ставим в tov_kontr_date текущей датой (S - sum(Значение)) >0

получилась таблица #tov_kontr_date_corr

 N_circle_1 = 0 - значит приоритет распределить остатки, тк старые, 1 - не старые

**/

--drop table #tov_kontr_date_corr

--Declare @N int = 16502 , @date_rasp date = '2019-07-20'
/**
select *
from #tov_kontr_date_corr c
where id_tov =46
**/


create table #tov_kontr_date_corr (id_tov int, id_kontr int, date_ost date, q_ost_sklad real, rn int, 
q_ost_sklad_narast real , Kolvo_korob real ,q_ost_sklad_init real , N_circle_1 int)

insert into #tov_kontr_date_corr
--Declare @N int = 16502
Select tkd.id_tov , tkd.id_kontr , tkd.date_ost , tkd.q_ost_sklad ,
ROW_NUMBER() over (partition by tkd.id_tov , tkd.id_kontr order by tkd.date_ost) rn , 
0 q_ost_sklad_narast , tk.Kolvo_korob , 0 , 1
from M2..tov_kontr_date tkd
inner join 
(select tk.*
from m2..tov_kontr tk
inner join #tov_new_algor tng on tng.id_tov = tk.id_tov
where tk.Number_r=@N 
--and tk.rasp_all = 0 
and tk.q_ost_sklad_calc>0) tk on  tkd.id_tov = tk.id_tov and tkd.id_kontr = tk.id_kontr
where tkd.Number_r=@N and tkd.q_ost_sklad>0

update #tov_kontr_date_corr 
set q_ost_sklad_narast = a.q
from #tov_kontr_date_corr c2
inner join
(
Select c2.id_tov , c2.id_kontr , c2.rn , SUM(c1.q_ost_sklad) q
from #tov_kontr_date_corr c1
inner join #tov_kontr_date_corr c2 on c1.id_tov = c2.id_tov and c1.id_kontr= c2.id_kontr and c2.rn > c1.rn 
group by c2.id_tov , c2.id_kontr , c2.rn
) a on c2.id_tov = a.id_tov and c2.id_kontr = a.id_kontr and c2.rn = a.rn


--Declare @N int = 16502
--select tkd.* , master.dbo.minz (tkd.q_ost_sklad , tk.q_ost_sklad - tkd.q_ost_sklad_narast) , tk.q_ost_sklad
update #tov_kontr_date_corr
set q_ost_sklad = 
case when tkd.q_ost_sklad_narast< tk.q_ost_sklad_calc then 
master.dbo.minz (tkd.q_ost_sklad , tk.q_ost_sklad_calc - tkd.q_ost_sklad_narast)
else 0 end
from #tov_kontr_date_corr tkd
left join 
(select tk.*
from m2..tov_kontr tk
inner join #tov_new_algor tng on tng.id_tov = tk.id_tov
...
--Declare @N int = 16502
--И оставшуюся разницу ставим в tov_kontr_date будущей датой (S - sum(Значение)) >0

insert into #tov_kontr_date_corr 
--Declare @N int = 16502
select  a.id_tov , a.id_kontr , '3000-01-01' , b.q - a.q,
0 rn , 
0 q_ost_sklad_narast , b.Kolvo_korob , 0 , 1
from
(select  tk.id_tov , tk.id_kontr , SUM(tk.q_ost_sklad) q
from #tov_kontr_date_corr tk 
group by  tk.id_tov , tk.id_kontr) a
inner join
(
select  tk.id_tov , tk.id_kontr ,tk.q_ost_sklad_calc q , tk.Kolvo_korob
from M2..tov_kontr (nolock) tk
inner join #tov_new_algor tng on tng.id_tov = tk.id_tov
where tk.Number_r=@N 
--and tk.rasp_all = 0 
and tk.q_ost_sklad_calc>0
) b on a.id_tov = b.id_tov and a.id_kontr = b.id_kontr
where b.q > a.q

delete from #tov_kontr_date_corr
where q_ost_sklad=0

update #tov_kontr_date_corr
set q_ost_sklad_init = q_ost_sklad --, N_circle_1 =1

--Declare @N int = 16502 , @date_rasp date = '2019-07-20'
-- первый проход по товарам со старым сроком годности  - полностью распределять
update #tov_kontr_date_corr
set N_circle_1=1
from #tov_kontr_date_corr c
inner join M2..tov_kontr tk on tk.Number_r = @N and tk.id_tov = c.id_tov and tk.id_kontr = c.id_kontr
inner join m2..tov on tov.Number_r = @N and tov.id_tov = c.id_tov
where 
datediff(day, c.date_ost , @date_rasp) > tk.srok_godnosti * 0.5
or tov.id_group in (65, 10174, 10176)
...
теперь из таблицы #tov_kontr_date_corr заполним q_rasp  в #rasp
 
1. сначала все, у кого на складе более 50% срока годности (0% - ФРОВ)
 - в первую очередь тем, у кого совпало
 - потом остальным
2. остальные - сначала  у кого совпало
- потом остальные



Цикл 1 - по Срокам - Какие включать, какие нет ( 1 - менее 50% или 0% для ФРОВ , 2 - прочие)
Цикл 2 - по Контрагенту -равен или нет ( 1 id_kontr  равны в tov_kontr_date и rasp, не важно)

Временная по остаткам #tov_kontr_date_corr - id_tov , id_kontr , date, q_ost  , N_rn . Для цикла условие по срокам
Временная по rasp #rasp_circle -  id_tt, id_tov , id_kontr , q_nuzno , Znach , Znach_narast , tt_rn  

Присваиваем номера для id_kontr, date - сортировка по увеличению срока годности N_rn, q>0 
Находим максимальный N_nr
Цикл 3 по N_rn , начиная с 1


Сортируем rasp  по уменьшению q_nuzno, добавляем Нарастающий , чтоб остался наименьший в конце, но чтоб (Знач / КолвоКороб) * КолвоКороб >= Остаток
Значение берем, как ceiling (Знач / КолвоКороб) * КолвоКороб
Знач Нараст
а         0
b        a
c        a+b

Все, что Нарастающий + Знач < Остаток -0.01 для N_rn распределяем.  Для цикла 3 условие на соответствие id_kontr
В наибольший распределенный добавляем разницу  Остаток - SUM(Знач) >0.01

Если (Знач / КолвоКороб) * КолвоКороб < Остаток
тогда добавляем с самый близкоподходящий под(Знач / КолвоКороб) * КолвоКороб к Остатку


убрать откругление до коробки q_nuzno и смену характеристики для rasp_all = 0
проверить, что реально хватает


проверить цену price_rasp

**/

--drop table #rasp_circle

--Declare @N int = 16502

create table #rasp_circle (id_tt int, id_tov int, id_kontr int, id_kontr_init int, q_nuzno real ,  znach real, q_nuzno_narast real,  tt_rn int , N_circle_1 int , N_circle_2 int , N_circle_3 int)


insert into #rasp_circle
SElect rasp.id_tt , rasp.id_tov , rasp.id_kontr_init , rasp.id_kontr_init, rasp.q_nuzno , 0 znach , 0 q_nuzno_narast , 
ROW_NUMBER () over (partition by rasp.id_tov , rasp.id_kontr_init order by rasp.q_nuzno desc) tt_rn , 0 , 0 ,0
from m2..rasp rasp
inner join 
(
select distinct c.id_tov
from #tov_kontr_date_corr c
) tk on rasp.id_tov = tk.id_tov
where rasp.number_r=@N
--     and @sql_raspr = 0 and @only_zakaz=0
and not (isnull(rasp.type_reserve,0) in (1,4)) -- значит это добавлены тт, которые уже отправлены
and rasp.q_nuzno>0	
...
update #rasp_circle
set q_nuzno_narast = 0 , znach = 0 , N_circle_1 = 0, N_circle_2 = 0 , N_circle_3 = 0


Declare @N_circle_1 int = 0, @N_circle_2 int = 0, @N_circle_3 int = 1, @N_circle_max_3 int


While @N_circle_1<=1 -- сначала плохие сроки, потом нормальные
begin


Select @N_circle_max_3 = 0 , @N_circle_3 =1

Select @N_circle_max_3 = MAX(c.rn)
from #tov_kontr_date_corr c
where c.N_circle_1 = @N_circle_1


while @N_circle_3 <=  @N_circle_max_3
begin


Select @N_circle_2 = 0 

While @N_circle_2<=1 -- сначала по совпавшим id_kontr, потом по любым
begin


-- пересортировать, что в rn были отсортированы по датам остатки
update #tov_kontr_date_corr 
set rn = b.rn2
from #tov_kontr_date_corr c
inner join
(
--declare @N_circle_2 int = 1
select * , ROW_NUMBER() over (partition by c.id_tov , case when @N_circle_2=0 then c.id_kontr else c.id_tov end  order by c.date_ost ) rn2
from #tov_kontr_date_corr c
) b on c.id_tov = b.id_tov and  c.id_kontr = b.id_kontr and c.date_ost = b.date_ost
where c.N_circle_1 = @N_circle_1 and c.rn = @N_circle_3


--update #rasp_circle
--set q_nuzno_narast = 0 

update #rasp_circle
set q_nuzno_narast = isnull(c.q_nuzno_narast,0) , znach = c.Znach ,  id_kontr = c.id_kontr ,
N_circle_1 = @N_circle_1, N_circle_2 = @N_circle_2, N_circle_3 = @N_circle_3 
from #rasp_circle r
inner join
(
select r2.id_tt , r2.id_tov  , c.id_kontr ,  r2.tt_rn ,  ceiling(r2.q_nuzno / c.Kolvo_korob) * c.Kolvo_korob Znach , 
SUM(ceiling(r.q_nuzno / c.Kolvo_korob) * c.Kolvo_korob) q_nuzno_narast
from #rasp_circle r2 
left join #rasp_circle r on r.id_tov = r2.id_tov and (r.id_kontr = r2.id_kontr or  @N_circle_2=1 )
and  r2.tt_rn > r.tt_rn and r.znach = 0  -- значит еще не распределены

inner join
(select c.id_tov , c.id_kontr , c.q_ost_sklad , c.Kolvo_korob
from #tov_kontr_date_corr c
where c.N_circle_1 = @N_circle_1 and c.rn = @N_circle_3
) c on c.id_tov = r2.id_tov and (c.id_kontr = r2.id_kontr or  @N_circle_2=1 )
where r2.znach=0  -- значит еще не распределены
group by r2.id_tt , r2.id_tov  , c.id_kontr  ,ceiling(r2.q_nuzno / c.Kolvo_korob) * c.Kolvo_korob , r2.tt_rn, c.q_ost_sklad 
having SUM(ceiling(isnull(r.q_nuzno,0) / c.Kolvo_korob) * c.Kolvo_korob) +  ceiling(r2.q_nuzno / c.Kolvo_korob) * c.Kolvo_korob <=  c.q_ost_sklad 
and ceiling(r2.q_nuzno / c.Kolvo_korob) * c.Kolvo_korob < c.q_ost_sklad * 0.9
) c on c.id_tt = r.id_tt and c.id_tov = r.id_tov
inner join M2..tt on tt.id_TT = r.id_tt
where (@N_circle_1 =0 and tt.tt_format in (2,12,400)) or @N_circle_1 =1 

--Declare @N_circle int = 1
-- найти разницу между тем, что распределили и тем, что на складе - отнести на самую большую q_nuzno
-- добавить в первую тт
--select *

-- 

update #rasp_circle
set znach = r.znach + c.q_d 
from #rasp_circle r
inner join
(select  r.id_tov , r.id_kontr  ,c.q_ost_sklad - sum(r.znach) q_d
from #rasp_circle r
inner join m2..tt on tt.id_TT = r.id_tt and tt.tt_format in (2,12,400)
inner join
(select c.id_tov , c.id_kontr , c.q_ost_sklad , c.Kolvo_korob
from #tov_kontr_date_corr c
where c.N_circle_1 = @N_circle_1 and c.rn = @N_circle_3
) c on c.id_tov = r.id_tov and c.id_kontr = r.id_kontr 
where  r.N_circle_1 = @N_circle_1 and  r.N_circle_2 = @N_circle_2 and  r.N_circle_3 = @N_circle_3
group by  r.id_tov , r.id_kontr  , c.q_ost_sklad, c.Kolvo_korob 
having  (c.q_ost_sklad - sum(r.znach)  )< MIN(r.znach) and c.q_ost_sklad - sum(r.znach)>0
) c on r.id_tov = c.id_tov
where r.tt_rn=1 and c.q_d < r.znach*0.5   
...
from #rasp_circle c
where id_tov =46
order by c.N_circle_1 , c.N_circle_3 , c.N_circle_2 
**/

-- найти остатки, которые не разнеслись, тк очень малы на первую тт
--Declare @N_circle int = 1

update #rasp_circle
set znach = c.q_ost_sklad , N_circle_3 = @N_circle_3
from #rasp_circle r
inner join
(select top 1 with ties r.id_tov , r.id_kontr , r.id_tt , c.q_ost_sklad -- выбираем магазин, который больше всего подходит
from #rasp_circle r
inner join
(select  c.*
from 
...
left join #rasp_circle r
on c.id_tov = r.id_tov and c.id_kontr = r.id_kontr  and r.N_circle_1 = @N_circle_1  and r.N_circle_2 = @N_circle_2 and r.N_circle_3 = @N_circle_3
where r.id_tov is null and r.q_nuzno>0 -- значит остатки есть, но ни на один магазине не пошли

) c on c.id_tov = r.id_tov and c.id_kontr = r.id_kontr
order by ROW_NUMBER() over (partition by r.id_tov , r.id_kontr  order by abs (r.q_nuzno - c.q_ost_sklad)  )
) c on  c.id_tov = r.id_tov and c.id_kontr = r.id_kontr and c.id_tt = r.id_kontr


-- снять с остатков распределенное

update #tov_kontr_date_corr
set q_ost_sklad = c.q_ost_sklad - r.znach
from #tov_kontr_date_corr c
inner join
(select r.id_tov , r.id_kontr , SUM(r.znach) znach
from #rasp_circle r
where r.N_circle_1 = @N_circle_1  and r.N_circle_2 = @N_circle_2 and r.N_circle_3 = @N_circle_3
group by r.id_tov , r.id_kontr
) r on c.id_tov = r.id_tov and c.id_kontr = r.id_kontr
where c.N_circle_1 = @N_circle_1 and c.rn=@N_circle_3


Select @N_circle_2 = @N_circle_2+1 
end

Select @N_circle_3 = @N_circle_3+1 
end

Select @N_circle_1 = @N_circle_1+1 
end

/**
select r.id_tov , r.id_kontr ,  sum(r.znach) , c.q_ost_sklad , c.q_ost_sklad_init,
sum(case when r.znach = 0 then r.q_nuzno else 0 end) , c.Kolvo_korob
from #rasp_circle r
inner join 
(select  c.id_tov , c.id_kontr , sum(c.q_ost_sklad) q_ost_sklad , sum(c.q_ost_sklad_init) q_ost_sklad_init , c.Kolvo_korob
from #tov_kontr_date_corr c 
group by c.id_tov , c.id_kontr , c.Kolvo_korob
) c
on c.id_tov = r.id_tov and c.id_kontr = r.id_kontr
group by r.id_tov , r.id_kontr ,c.q_ost_sklad , c.q_ost_sklad_init , c.Kolvo_korob
...
select *
from #tov_kontr_date_corr c
order by c.id_tov , c.date_ost


select *
from #rasp_circle r
where r.id_tov =16306 --and r.id_kontr=16628
order by r.N_circle_1 , r.N_circle_2 , r.N_circle_3
...
**/
	 ...
	 insert into #rasp
	 select 
	 @N number_r, rasp.id_tov , rasp.id_tt , rasp.id_kontr , -1, 
	 rasp.znach , N_circle_1 ,  -1 prohod, N_circle_2 sort_ost , N_circle_3  sort_pr , tt_rn p1 , 0 , 0 , 0 rn_gr, NULL , 
	 r.tt_format_rasp,   
	 case when r.koef_ost_pr_rasp is not null then tov.id_group_rasp end id_group_rasp , 
     r.price_rasp, 
     r.koef_ost_pr_rasp
	 from #rasp_circle rasp
	 inner join m2..rasp r on r.number_r = @N and r.id_tt = rasp.id_tt and r.id_tov = rasp.id_tov
	 inner join m2..tov tov on tov.Number_r = @N and  tov.id_tov = rasp.id_tov
...
---------------------------------------------------------------------------------------------
	...
	update M2..rasp 
	set q_raspr = rasp.q_raspr + rasp1.znach , q_ko_ost = rasp.q_ko_ost +rasp1.znach , id_kontr = rasp1.id_kontr
	--select q_raspr , rasp1.znach , q_ko_ost, rasp.q_ko_ost +rasp1.znach
	from M2..rasp with (rowlock, index(ind1))
	inner join #rasp rasp1 on rasp1.id_tt=rasp.id_tt and rasp1.id_tov=rasp.id_tov
	where rasp.number_r=@N
	...
	update M2..tt_tov_kontr
	set id_kontr = rasp1.id_kontr
	--select q_raspr , rasp1.znach , q_ko_ost, rasp.q_ko_ost +rasp1.znach
	from M2..tt_tov_kontr ttk with (rowlock, index(ind1))
	inner join #rasp rasp1 on rasp1.id_tt=ttk.id_tt and rasp1.id_tov=ttk.id_tov
	where ttk.number_r=@N and ttk.id_kontr <> rasp1.id_kontr
 ...
 -- АК - смотрим, а как было в прошлом распределении 
   ...
   create unique clustered index ind1 on #raspr_last (id_tt , id_tov)
	...
	, rasp.tt_format_rasp  , rasp.price_rasp , rasp.koef_ost_pr_rasp , rasp.type_reserve , rasp.q_min_ost
	into #rasp9701
	from M2..tov_kontr tk 	with ( INDEX(IX_tov_kontr_1))
...
--  Вторые блюда	10223
	...
	--drop table #tovgr  
	create table #tovgr  (id_tov int , id_group int,  price int , id_group_init int , all_nesklad int) -- koef_ost_pr int,
	insert into #tovgr
	select distinct t.id_tov , t.Group_raspr  , pr.Price , t.id_group , case when a.Group_raspr is not null then 1 else 0 end
    from m2..Tovari t 
    ...
    left join
     (
     select t.Group_raspr 
from M2..Tovari t
inner join M2..Group_koef_raspr gr on gr.id_group = t.Group_raspr
where t.id_group in (65, 10174, 10176)
group by t.Group_raspr
having SUM ( t.skladir_tov )=0 -- чтоб не было КолвоСкадир
     ) a on a.Group_raspr = t.Group_raspr  
	...
	when rasp.tt_format_rasp in (4, 6, 12, 14,16,18,300,400)  then 0 else 1 end, -- приоритет с @ff=1
	...
	or rasp.tt_format_rasp in (4, 6, 12, 14,16,18,300,400)  then 0 else 1 end, -- приоритет с  @ff=1
   ...
   ,rasp.tt_format_rasp  , 
	...
	left join m2..tt_Type_Rounding ttr on ttr.id_tt = rasp.id_tt
	...
	and (( rasp.tt_format_rasp<>10 and (
	not (rasp.tt_format_rasp in (4,14,16)  and w.type_pr=1  and rasp.srok_godnosti<8 
	and not (tt.N in (3069,2014,2015,2186,2120) and datepart(weekday,@date_rasp)=6)
	)
	...
	or ( rasp.tt_format_rasp in (4,14,16)  and w.type_pr=1  and rasp.srok_godnosti<8 
	 and not (tt.N in (3069,2014,2015,2186,2120) and datepart(weekday,@date_rasp)=6)
	 and rasp.q_ko_ost + korob* rasp.Kolvo_korob <= 0.3 * rasp.Kolvo_korob  )
	 
	or  (rasp.tt_format_rasp=10 
	and korob * rasp.Kolvo_korob + rasp.q_raspr <=rasp.q_min_ost + case when ttr.ТипОкруг=1 then rasp.Kolvo_korob-0.01 else 0.01 end   )) 
	)
    ...
    and not ( rasp.tt_format_rasp not in (2,400) and  (@hour_raspr between 4 and 7 or t_vv.id_tov is not null) and tov.raspr_double=1 ) -- не распределять иные форматы, если утром или будет еще распр и скоропорт
...
--	select *
--	from #rasp9702 r
--	where r.id_tt = 12341 and r.id_tov = 1668
	...
	p1 , znach* pr.Price znach_sum, 0 znach_sum_narast , rasp.rn_gr rn_gr ,
	...
	, tt_format_rasp  , id_group_rasp , pr.Price, rasp.koef_ost_pr_rasp
	from #rasp9702 rasp
...
--1 (все), 2 (без полных аналогов), 3 (только характеристика из ассортиментной матрицы)

-- добавить все Маркетплейсы, у которых только характеристика из матрицы


--select *
--from [M2].[dbo].[Kontr_id_tt_type_rasp] ki

--truncate table #rasp_Kontr_iskl
-- Declare @N int =23468
insert into #rasp_Kontr_iskl
select distinct ki.id_tt , r.id_tov , 0, 0 
from [M2].[dbo].[Kontr_id_tt_type_rasp] ki
inner join m2..rasp r on r.id_tt = ki.id_tt and r.number_r = @N
where ki.type_rasp=3
...
/**
 ...
 -- добавить 12763	Сторонняя розница	2458МП_ЯНДЕКС_БЕРУ
 insert into #rasp_Kontr_iskl
 select distinct 12763 , 0 , 0 , 0
...
select distinct ki.id_tt 
into #tt_type_rasp_2
from [M2].[dbo].[Kontr_id_tt_type_rasp] ki
inner join m2..rasp r on r.id_tt = ki.id_tt and r.number_r = @N
where ki.type_rasp=2
 ...
 --Declare @N int = 4456
 -- Declare @N int =23468
 select distinct r.id_tt , r.id_tov , tk.id_kontr , 0 --, r.id_kontr_init
 from m2..rasp r
 inner join 
 (
	 select distinct rk.id_tt from #rasp_Kontr_iskl rk 
	 union	
	 select id_tt 
	 from #tt_type_rasp_2
 ) rk on rk.id_tt=r.id_tt

 left join m2..tt_tov_kontr ttk on ttk.Number_r = @N and ttk.id_tt = r.id_tt and ttk.id_tov = r.id_tov and ttk.id_kontr = r.id_kontr  

 left join m2..tov_kontr tk0 on tk0.Number_r = @N and tk0.id_tov = r.id_tov and tk0.id_kontr = isnull(ttk.id_kontr_init, r.id_kontr_init) -- первоначальная характеристика
 
 inner join m2..tov_kontr tk on tk.Number_r = @N and tk.id_tov = r.id_tov  -- не равен тому, что был первоначально - те уброать полные аналоги
 
 left join #rasp_Kontr_iskl rki on rki.id_tov = r.id_tov and rki.id_tt = r.id_tt and rki.id_kontr = tk.id_kontr
 
 inner join 
 (select  ki.id_tt  , max(ki.type_rasp) type_rasp
from [M2].[dbo].[Kontr_id_tt_type_rasp] ki	
where ki.type_rasp in (2,3) 
group by ki.id_tt ) ki on ki.id_tt = r.id_tt
 ...
 and (  ( not (tk.id_kontr = isnull(ttk.id_kontr_init, r.id_kontr_init) and isnull(tk0.id_tov_init,-1)= tk.id_tov_init) and ki.type_rasp=3)  --добавить все, у которых или полный аналог или другая характеристика
 or     (                                    isnull(tk0.id_tov_init,-1)<> tk.id_tov_init   and ki.type_rasp=2) ) -- добавить только тех,  у кого полная характеристика
 

 -- добавить товары, которые старые на остатках для отгрузки в МП
  insert into #rasp_Kontr_iskl
 
 --Declare @N int =24641 , @date_rasp date = '2019-09-14'
 select  distinct r.id_tt , r.id_tov , tk.id_kontr , 0 --, r.id_kontr_init
  
 from m2..tov_kontr tk
 inner join m2..rasp r on r.number_r = tk.Number_r and tk.id_tov = r.id_tov 
 inner join izbenkafin..AllowableTermLife at0 on at0.id_tt = r.id_tt and isnull(tk.srok_godnosti_fact, tk.srok_godnosti) between at0.SrokOt and at0.SrokDo
 inner join m2..tov_kontr_date tkd on tkd.number_r = tk.Number_r and tkd.id_tov = tk.id_tov and tkd.id_kontr = tk.id_kontr
 
 left join #rasp_Kontr_iskl rki on rki.id_tov = r.id_tov and rki.id_tt = r.id_tt and rki.id_kontr = tk.id_kontr

 where tk.Number_r= @N and rki.id_tov is null-- and tkd.q_ost_sklad>r.q_min_ost
 group by r.id_tt , r.id_tov , tk.id_kontr, master.dbo.maxz(floor(0.01 * (100-at0.Procent) *  isnull(tk.srok_godnosti_fact, tk.srok_godnosti)), 1)
 having min(datediff(day,isnull(tkd.date_ost_fact, tkd.date_ost),@date_rasp)) >  master.dbo.maxz(floor(0.01 * (100-at0.Procent) *  isnull(tk.srok_godnosti_fact, tk.srok_godnosti)), 1)
 ...
 ) tk on ri.id_tov = tk.id_tov  and ri.id_tt = tk.id_tt
 
  
 
 --select * from #rasp_Kontr_iskl where id_tov = 17826
  --Declare @N int = 23468
 update #rasp
 ...
 --inner join m2..tov_kontr tk2 on tk2.Number_r = @N and tk2.id_tov= r.id_tov and tk2.id_kontr = r.id_kontr
 where isnull(ri.id_kontr_new,0) > 0


  --Declare @N int = 23468 
 update m2..rasp
 set id_kontr = ri.id_kontr_new 
 
/**
 , q_raspr = 
 
 case ttr.ТипОкруг when 1 then 
 ceiling((r.q_min_ost) / tk.Kolvo_korob - 0.01) * tk.Kolvo_korob 
 else
 floor((r.q_min_ost) / tk.Kolvo_korob+0.01) * tk.Kolvo_korob 
 end
 **/
 --select *
 from m2..rasp r
 ...
 --inner join m2..tov_kontr tk2 on tk2.Number_r = @N and tk2.id_tov= r.id_tov and tk2.id_kontr = r.id_kontr
 where isnull(ri.id_kontr_new,0) > 0 and r.number_r = @N and  r.id_kontr <> ri.id_kontr_new
 ...
 case ttr.ТипОкруг when 1 then 
 ceiling(master.dbo.minz(r.q_min_ost, r.q_raspr) / tk.Kolvo_korob - 0.01) * tk.Kolvo_korob 
 else
 floor(master.dbo.minz(r.q_min_ost, r.q_raspr) / tk.Kolvo_korob + 0.01) * tk.Kolvo_korob 
 end

 from m2..rasp r
 inner join m2..tov_kontr tk on tk.Number_r = @N and tk.id_tov= r.id_tov and tk.id_kontr = r.id_kontr
 inner join m2..tt_Type_Rounding ttr on ttr.id_tt = r.id_tt

 where isnull(r.id_kontr,0) > 0 and r.number_r = @N 
   
 -- удалить или добавить коробки, если квант другой
 ...
 from #rasp r
 inner join 
 (  select distinct ri.id_tt , ri.id_tov
  from #rasp_Kontr_iskl ri
  where isnull(ri.id_kontr_new,0) =0) ri on r.id_tt = ri.id_tt and r.id_tov = ri.id_tov 
...
set q_raspr = 0
 ...
 --select * from #rasp_Kontr_iskl order by id_tov
...
/*
 -- удалить все занесенные в rasp
 delete #rasp
 from #rasp r
 inner join 
 (  select distinct ri.id_tt , ri.id_tov
  from #rasp_Kontr_iskl ri
  ) ri on r.id_tt = ri.id_tt and r.id_tov= ri.id_tov 
  */
...
create table #zakaz_tt_zabrat (id_z int, id_tt int, id_tov int, id_kontr int, id_tt_new int, prohod int, id_kontr_new int, q_raspr real , id_tov_init int)
...
-- АК добавил еще 0 цикл - в нем отрабатываются только заказы покупателям ММ. самого распределения нет
...
and  r.tt_format_rasp in (4, 6, 12, 14,16,18,300,400)
group by r.id_tov) 
...
--if @only_zakaz=1
--select @ff=2 -- если только заказы покупателей, то сразу на 2
...
if @ff =0 -- цикл для распределение ММ по заказам покупателей
begin

insert into #rasp
select * 
from #rasp_0 r
where r.tt_format_rasp in (4) and r.koef_ost_pr_rasp is not null
...
select *
from #rasp_0 r
where r.tt_format_rasp in (4, 6, 12, 14,16 ,18, 300,400) and r.koef_ost_pr_rasp is not null
...
select *
from #rasp_0 r
where not ( r.tt_format_rasp in (4, 6, 12, 14,16,18,300,400) and r.koef_ost_pr_rasp is not null )
...
if @ff>0 -- чтоб в само распределение не заходило на шаге 0 - распределение ММ по заказам покупателей
if @only_zakaz=0
begin



-- пересчитать sort_r - это  сортировка по partition by rasp.id_tov , rasp.id_kontr

--select * from #rasp

update #rasp
set sort_r = a.sort_r_2
from #rasp rasp
inner join
(select * ,
ROW_NUMBER() over (partition by rasp.id_tov , rasp.id_kontr, rasp.prohod order by sort_r) sort_r_2
from #rasp rasp
) a on a.id_tt = rasp.id_tt and a.id_tov = rasp.id_tov  and rasp.prohod = a.prohod 
...
from 
	...
	(
      ...
      where rasp.number_r=  @N and rasp.koef_ost_pr_rasp is not null and rasp.id_kontr_init is not null
...
update #rasp_gr_reestr
set id_kontr = r2.id_kontr  , Распр = 0  , Сняли = r1.Распр ,id_tt_old = @id_tt_v , type_add_kor = 2000
...
from #rasp r2
inner join m2..rasp r on r.number_r = @N and r2.id_tt = r.id_tt and r2.id_tov = r.id_tov and r.id_kontr_init is not null
...
set id_kontr = r2.id_kontr
from #rasp r2
inner join m2..tt_tov_kontr r on r.number_r = @N and r2.id_tt = r.id_tt and r2.id_tov = r.id_tov and r.id_kontr_init is not null
where r2.id_kontr <> r.id_kontr

/**

Сообщение 2627, уровень 14, состояние 1, строка 9229
Нарушение "PK_rasp" ограничения PRIMARY KEY. Не удается вставить повторяющийся ключ в объект "dbo.rasp". Повторяющееся значение ключа: (17805, 11816, 20542, 13990).
Выполнение данной инструкции было прервано.
Сообщение 2627, уровень 14, состояние 1, строка 9236
Нарушение "PK_tt_tov_kontr" ограничения PRIMARY KEY. Не удается вставить повторяющийся ключ в объект "dbo.tt_tov_kontr". Повторяющееся значение ключа: (17805, 11816, 20542, 13990).


select *
from m2..rasp r
where r.number_r = 17805 --and id_tt = 11816 and id_tov = 20542
and id_kontr_init is null
 ...
 **/ 
 ...
 --select * from #b_9704 
  ...
  --where id_tt=12385
    ...
    from 
     ...
     (select r1.id_tt , r1.rn_gr , t.id_group , t.id_group_init , t.all_nesklad , SUM(r2.znach_sum) СуммаРасп
     from #rasp r1
     ...
     group by r1.id_tt , r1.rn_gr , t.id_group, t.id_group_init , t.all_nesklad
     ...
     --declare @N int = 83096 , @date_rasp date = '2018-12-10'   
      ...
      and ( (a.id_group_init in (65, 10174, 10176) and a.all_nesklad=0 ) or @dont_use_wait_sklad=1  or @ff=1)  -- только ФРОВ или Ра
    ...
    from 
     (
     select r1.id_tt , r1.id_group , id_group_init,  MAX(СуммаРасп) СуммаРасп , MAX(rn_gr) rn_gr, r1.all_nesklad
     from 
     (select r1.id_tt , r1.rn_gr , t.id_group , t.id_group_init ,SUM(r2.znach_sum) СуммаРасп , t.all_nesklad
     from #rasp r1
     inner join #tovgr t  on r1.id_tov = t.id_tov 
     inner join #rasp r2 on r1.id_tt = r2.id_tt and r2.rn_gr<=r1.rn_gr
     inner join #tovgr t2  on r2.id_tov = t2.id_tov  and t.id_group = t2.id_group
     ...
     group by r1.id_tt , r1.id_group, id_group_init, r1.all_nesklad
     ) a
     inner join 
     -- осталось распределить - не более
     --declare @N int = 83096 , @date_rasp date = '2018-12-10'   
      ...
      and ( (a.id_group_init in (65, 10174, 10176) and a.all_nesklad=0 ) or @dont_use_wait_sklad=1 or @ff=1)  -- только ФРОВ
  ...
  **/
	...
	select @getdate = getdate() 
      ...
      and ( (t.id_group_init in (65, 10174, 10176) and t.all_nesklad=0 )or @dont_use_wait_sklad=1 or @ff=1)  -- только ФРОВ
...
left join #rasp r1 on r.id_tt = r1.id_tt and r.id_tov = r1.id_tov
where r.number_r=@N and @sql_raspr=0 and r.q_raspr=0
and (r1.id_tt is not null or @ff=2)
...
(Select top 1 r.id_z
from M2..rasp_zakaz_pok  r
left join #rasp r1 on r1.id_tt = r.id_tt and r1.id_tov = r.id_tov
where r.number_r=@N and  r.q_raspr=0
and r.id_z>@id_z
and (r1.id_tt is not null or @ff=2)
order by  id_z 
...
group by r.id_tt , r.id_tov_pvz , isnull(r.id_kontr_new,r.id_kontr)
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706001, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 

-- добавить просмотр остатков на тт, если товар долгосрочный

update M2..rasp_zakaz_pok
set q_raspr=rzp.q , q_raspr_cur = rzp.q, type_add = 7
...
inner join m2..tov_kontr tk on tk.Number_r = ra.number_r and tk.id_tov = ra.id_tov and tk.id_kontr = ra.id_kontr
...
and ra.q_ko_ost - rzp.q  - isnull(zr1.q_raspr_cur,0)> tk.kolvo_korob * 0.5
...
begin
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706002, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 


update M2..rasp_zakaz_pok
set q_raspr=rzp.q , q_raspr_cur = rzp.q
 * case when mt1.Ves <> mt2.Ves and (mt1.Ves=1 or mt2.ves =1) then mt1.Ves/mt2.ves  else 1 end , 
  id_kontr_new = case when rzp.id_kontr <>ra.id_kontr_new then ra.id_kontr_new else Null end, type_add = 8
, id_tov_new = case when ra.id_tov <> ra.id_tov_init then ra.id_tov_init else Null end
from M2..rasp_zakaz_pok  rzp
inner join
(Select top 1 with ties rzp.id_z , rzp.id_tt, rzp.id_tov , rzp.id_kontr , ra.id_kontr id_kontr_new , tk.id_tov_init
...
order by row_number() over (partition by rzp.id_z order by (ra.q_ko_ost - rzp.q - isnull(zr1.q_raspr_cur,0)- tk.kolvo_korob * 0.5 ) desc )
) ra on ra.id_z = rzp.id_z --ra.id_tt = rzp.id_tt and ra.id_tov = rzp.id_tov and ra.id_kontr = rzp.id_kontr

inner join m2..tov mt1 on mt1.Number_r =@N and mt1.id_tov = ra.id_tov
inner join m2..tov mt2 on mt2.Number_r =@N and mt2.id_tov = ra.id_tov_init
...
begin
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706003, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
begin
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706004, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
set q_raspr=rzp.q , q_raspr_cur = rzp.q
 * case when mt1.Ves <> mt2.Ves and (mt1.Ves=1 or mt2.ves =1) then mt1.Ves/mt2.ves  else 1 end , 
  id_kontr_new = case when rzp.id_kontr <>ra.id_kontr_new then ra.id_kontr_new else Null end, type_add = 2
, id_tov_new = case when ra.id_tov <> ra.id_tov_init then ra.id_tov_init else Null end
from M2..rasp_zakaz_pok  rzp
inner join
(Select top 1 with ties rzp.id_z , rzp.id_tt, rzp.id_tov , rzp.id_kontr , ra.id_kontr id_kontr_new , tk.id_tov_init
...
begin
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706005, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
--declare @N int= 3522, @date_rasp date = '2019-04-17'
	...
	floor((tov_kontr.q_ost_sklad - case when w.id_tov is not null then  tov_kontr.q_wait_sklad else 0 end 
	- isnull(r_u.q_raspr,0)) / Kolvo_korob +0.001) ОсталосьРаспр
	from
	...
	left join #wait w on w.id_tov = tov_kontr.id_tov
	
	--and tov_kontr.rasp_all=1
	where number_r=@N 
	and floor((tov_kontr.q_ost_sklad  - case when w.id_tov is not null then  tov_kontr.q_wait_sklad else 0 end
	  - isnull(r_u.q_raspr,0)) / Kolvo_korob +0.001)>0


	      insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706006, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
and r.ThisManufacturer=1
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706007, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
begin
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706008, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
--Тип 4 -- замена контрагента
 ...
 --declare @N int = 11660
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706009, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 

--declare @N int= 3522, @date_rasp date = '2019-04-17'
-- добавить в #rasp товары, есть на складе по 1 коробке

-- declare @N int = 11660 , @id_z int = 76491

update M2..rasp_zakaz_pok
set q_raspr=ra.kolvo_korob 
* case when mt1.Ves <> mt0.Ves and (mt1.Ves=1 or mt0.ves =1) then mt1.Ves/mt0.ves  else 1 end
, q_raspr_cur = ra.kolvo_korob 
 * case when mt1.Ves <> mt2.Ves and (mt1.Ves=1 or mt2.ves =1) then mt1.Ves/mt2.ves  else 1 end
 , id_kontr_new  = case when rzp.id_kontr <>ra.id_kontr_new then ra.id_kontr_new else Null end, type_add = 4
, id_tov_new = case when ra.id_tov <> ra.id_tov_init then ra.id_tov_init else Null end

-- declare @N int = 11660 , @id_z int = 76491
-- select ra.kolvo_korob 
-- * case when mt1.Ves <> mt2.Ves and (mt1.Ves=1 or mt2.ves =1) then mt1.Ves/mt2.ves  else 1 end , *
from M2..rasp_zakaz_pok  rzp
inner join
--declare @N int= 3522, @date_rasp date = '2019-04-17'
(select  top 1 with ties r.id_z , r.id_tt, r.id_tov , r.id_kontr , ost_r.id_kontr id_kontr_new , tk.Kolvo_korob , tk.id_tov_init
from M2..rasp_zakaz_pok  r  -- одна строка, что в цикле
inner join #ost_r ost_r on ost_r.id_tov=r.id_tov_pvz --and ost_r.id_kontr=r.id_kontr
...
inner join m2..tov mt0 on mt0.Number_r =@N and mt0.id_tov = ra.id_tov
inner join m2..tov mt1 on mt1.Number_r =@N and mt1.id_tov = rzp.id_tov_pvz
inner join m2..tov mt2 on mt2.Number_r =@N and mt2.id_tov = ra.id_tov_init


    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706010, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 


insert into #rasp ( number_r ,id_tov , id_tt , id_kontr , rn_r , znach , sort_r ,  prohod , sort_ost  , sort_pr  ) 
select @N , r.id_tov_pvz ,r.id_tt , isnull(r.id_kontr_new,r.id_kontr), 1 , r.q_raspr_cur
 * case when mt1.Ves <> mt2.Ves and (mt1.Ves=1 or mt2.ves =1) then mt2.Ves/mt1.ves  else 1 end
 , 0 , isnull(ra.prohod,0)+1 , 0 , 77888
from M2..rasp_zakaz_pok  r  -- одна строка, что в цикле
left join (
select r.id_tt , r.id_tov , max(prohod)  prohod
from #rasp r 
...
inner join m2..tov mt1 on mt1.Number_r =@N and mt1.id_tov = r.id_tov_pvz
inner join m2..tov mt2 on mt2.Number_r =@N and mt2.id_tov = ISNULL(r.id_tov_new,r.id_tov)
...
begin
	...
	insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706011, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
select top 1 with ties r.id_z, r.id_tt ,  r.id_tov_pvz , r.id_kontr , ra.id_tt , ra.prohod , ra.id_kontr   , ra.znach , null
from M2..rasp_zakaz_pok  r  -- одна строка, что в цикле
...
order by row_number() over (partition by r.id_z order by case when ra.tt_format_rasp =10 then 1 else 0 end,  isnull(rgt.Снять_еще , 0 ) desc, ra.prohod  desc )
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706012, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
group by r.id_tt , r.id_tov
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706013, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
begin
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706014, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
(select *
from M2..rasp_zakaz_pok  r
where r.id_z = @id_z and r.id_tov =26859 and r.id_tt = 11216 )
...
select *
from  M2..tov_kontr r
where r.id_tov = 26859 and Number_r = 17805 
...
where  r.id_tov = 26859 and 
Number_r = 17805 

select *
from  #rasp r
where r.id_tov = 26859

select distinct r.id_tt , r.id_tov_pvz id_tov 
from M2..rasp_zakaz_pok  r 
where r.number_r=17805 

**/


--declare @N int= 3522, @date_rasp date = '2019-04-17'
--declare @N int= 17805 , @id_z int = 327834
truncate table #zakaz_tt_zabrat
insert into #zakaz_tt_zabrat


select top 1 with ties r.id_z, r.id_tt ,  r.id_tov_pvz , r.id_kontr , ra.id_tt , ra.prohod , ra.id_kontr   , ra.znach , tk.id_tov_init
from M2..rasp_zakaz_pok  r  -- одна строка, что в цикле

inner join #rasp ra on ra.id_tov=r.id_tov_pvz --and ra.id_kontr=r.id_kontr --and ra.prohod <>1 -- не трогать пепрвую коробку
--and ra.tt_format_rasp=2

inner join m2..tov_kontr tk on tk.Number_r=@N and tk.id_tov = ra.id_tov and tk.id_kontr = ra.id_kontr -- and tk.id_tov_init = r.id_tov
...
isnull(rgt.Снять_еще , 0 ) desc, ra.prohod  desc )
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706015, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
update M2..rasp_zakaz_pok 
set q_raspr = r.q_raspr 
 * case when mt1.Ves <> mt0.Ves and (mt1.Ves=1 or mt0.ves =1) then mt1.Ves/mt0.ves  else 1 end
, q_raspr_cur = r.q_raspr 
 * case when mt1.Ves <> mt2.Ves and (mt1.Ves=1 or mt2.ves =1) then mt1.Ves/mt2.ves  else 1 end
 , type_add = 6 , id_kontr_new  = case when rzp.id_kontr <>r.id_kontr_new then r.id_kontr_new else Null end 
, id_tt_old = r.id_tt_new
, id_tov_new = case when rzp.id_tov <> r.id_tov_init then r.id_tov_init else Null end
from M2..rasp_zakaz_pok  rzp
inner join #zakaz_tt_zabrat r on r.id_z = rzp.id_z
inner join m2..tov mt0 on mt0.Number_r =@N and mt0.id_tov = rzp.id_tov_pvz
inner join m2..tov mt1 on mt1.Number_r =@N and mt1.id_tov = r.id_tov
inner join m2..tov mt2 on mt2.Number_r =@N and mt2.id_tov = r.id_tov_init

    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706016, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 


-- добавить в  магазин с заказом
insert into #rasp ( number_r ,id_tov , id_tt , id_kontr , rn_r , znach , sort_r ,  prohod , sort_ost  , sort_pr  ) 
select @N , r.id_tov ,r.id_tt , isnull(r.id_kontr_new, r.id_kontr), 1 , rzp.q_raspr_cur
 * case when mt1.Ves <> mt2.Ves and (mt1.Ves=1 or mt2.ves =1) then mt2.Ves/mt1.ves  else 1 end 
, 0 , isnull(ra.prohod,0)+1 , 0 , 77890
from #zakaz_tt_zabrat  r  -- одна строка, что в цикле
inner join M2..rasp_zakaz_pok  rzp on rzp.id_z = r.id_z
left join (
select r.id_tt , r.id_tov , max(prohod)  prohod
from #rasp r 
group by r.id_tt , r.id_tov
...
inner join m2..tov mt1 on mt1.Number_r =@N and mt1.id_tov = rzp.id_tov_pvz
inner join m2..tov mt2 on mt2.Number_r =@N and mt2.id_tov = ISNULL(rzp.id_tov_new,rzp.id_tov)

    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706017, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
inner join #rasp_gr_tt rgt on rgt.id_tt = r.id_tt_new and rgt.id_group_rasp = tg.id_group
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706018, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
from M2..rasp_zakaz_pok  rzp
left join m2..tov_kontr tk on tk.Number_r=rzp.Number_r and tk.id_tov = rzp.id_tov_pvz and tk.id_kontr = rzp.id_kontr_new
where rzp.id_kontr_new is not null and rzp.id_z = @id_z
and (rzp.kolvo_korob <> tk.Kolvo_korob or rzp.id_tov_new is not null))
begin

-- declare @N int = 11660 , @id_z int = 76712 , @QZ real 

select @QZ= 
master.dbo.maxz(0,b.q - sum(rzp.q_raspr))  -- Заказ Покупателя, осталось собрать
--master.dbo.maxz(0,sum(rzp.q) - SUM(rzp.q_raspr))  -- Заказ Покупателя, осталось собрать
from M2..rasp_zakaz_pok  rzp
inner join #q_zak b  on @N = rzp.Number_r and b.id_tt=rzp.id_tt and b.BonusCard= rzp.BonusCard and b.id_tov = rzp.id_tov and b.id_kontr= rzp.id_kontr and b.ThisManufacturer = rzp.ThisManufacturer
inner join
(select rzp.Number_r ,rzp.id_tt , rzp.BonusCard , rzp.id_tov , rzp.id_kontr, rzp.ThisManufacturer 
from M2..rasp_zakaz_pok  rzp
--inner join m2..tov_kontr tk on tk.Number_r=rzp.Number_r and tk.id_tov = rzp.id_tov and tk.id_kontr = rzp.id_kontr_new
where rzp.id_kontr_new is not null and rzp.id_z = @id_z
) a on a.Number_r = rzp.Number_r and a.id_tt=rzp.id_tt and a.BonusCard= rzp.BonusCard and a.id_tov = rzp.id_tov and a.id_kontr= rzp.id_kontr  and a.ThisManufacturer = rzp.ThisManufacturer
group by b.q 


    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706019, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
where rzp.q_raspr=0
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706020, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
td.N_korob  + k.Korob , ISNULL(tov.id_tov_pvz , tov.id_tov) , 0
...
or (k.Korob=1 and tk.Kolvo_korob is null ) 
    ...
    insert into jobs..Jobs_log ([id_job],[number_step],[duration] , par1) 
	select @id_job , 9706021, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) , @id_z
	select @getdate = getdate() 
...
-- сохранить во временную, тк после добавления потеряется уникальность
if OBJECT_ID('tempdb..#ttk_rasp') is not null drop table #ttk_rasp
...
left join #ttk_rasp ttk on  ttk.id_tt = r.id_tt and ttk.id_tov = r.id_tov --and ttk.id_kontr = r.id_kontr
inner join M2..tov  t on t.Number_r = @N and r.id_tov = t.id_tov
where r2.number_r is null

	insert into jobs..Jobs_log ([id_job],[number_step],[duration]) 
	select @id_job , 981, DATEDIFF(MILLISECOND , @getdate ,GETDATE()) 
	select @getdate = getdate() 
...
select distinct @N , r.id_tt , r.id_tov , r.id_kontr , isnull(ttk.q_FO,0) , isnull(ttk.q_plan_pr,0) , isnull(ttk.q_min_ost,0), isnull(ttk.max_ost_tt_tov,0) , 0 
 ,tt.tt_format , r.price_rasp , r.koef_ost_pr_rasp  , ISNULL(t.id_tov_pvz,t.id_tov)
from #rasp r
left join m2..rasp r2 on r2.number_r = @N and r2.id_tt = r.id_tt and r2.id_tov = r.id_tov and r2.id_kontr = r.id_kontr
left join #ttk_rasp ttk on ttk.id_tt = r.id_tt and ttk.id_tov = r.id_tov --and ttk.id_kontr = r.id_kontr
inner join M2..tov t on t.Number_r = @N and r.id_tov = t.id_tov
...
**/
	...
	begin
		insert into #id_tt_Макс_кор
		SELECT id_tt
			,case when SUM(summa) / COUNT(DISTINCT CONVERT(date, date_tt)) <200000 
				  then 0 
				  else convert(int,1.0 * SUM(summa) / COUNT(DISTINCT CONVERT(date, date_tt)) / 1000 / 10) end AS Макс_кор
		FROM Reports..DT AS dtt   
		WHERE date_tt between DATEADD(week,-2,convert(date,getdate())) and 	DATEADD(day,-3,convert(date,getdate()))
		GROUP BY id_tt
		 ...
		 inner join M2..tt tt
			on tt.id_TT = tm.id_tt 
				and (tt.tt_format in ( 4,14,16) or tt.ТорговаяПлощадь<100)
...
---------------------------------------------------------------------------------------------------
   ...
   -- if OBJECT_ID ('tempdb..#IRg4020') is not null drop table #IRg4020
	...
	--declare @N int =12165 , @date_rasp date = {d'2019-06-13'}, @dont_use_wait_sklad int = 0 , @sql_raspr int =0, @only_zakaz int = 0

	insert into #rasp	
	Select @N , a.id_tov,a.id_tt , a.id_kontr , 4 , a.Kolvo_korob , a.rn , a.N_kor, a.Fact , a.ОсталосьРаспр , 0 p1 ,0 , 0 ,0, 0 ,
	 0  , 0  ,0,  0
	from
	(select r.id_tov , r.id_tt , o.id_kontr , o.ОсталосьРаспр , r.Fact , tk.Kolvo_korob  ,
	ROW_NUMBER () over ( partition by o.id_tov , o.id_kontr 
	order by 
	k.N_kor ,
	case when isnull(r2.zc_status,4)=4 then 1 when r2.zc_status=5 then 2 else 3 end  , -- чтоб избытки сначала распределять по 4 статусу, потом 5, а потом уже 2
	 r.Fact - (r2.q_FO + r2.q_raspr) desc , isnull(rh.q_kor_add,0) ,tm.Макс_кор desc ) rn
	  ,
	  ROW_NUMBER() over ( PARTITION by r.id_tov , r.id_tt order by  k.N_kor ) N_kor 
	...
	inner join m2..rasp r2 with (index (PK_rasp) ) 
	 on r.id_tov=r2.id_tov and r.id_tt=r2.id_tt and r2.Number_r=@N and o.id_kontr  = r2.id_kontr and r2.tt_format_rasp not in (4,6,12,16,18,400)
    ...
    inner join #tovgr tov on tov.id_tov = o.id_tov
    
    --left join M2..Group_koef_raspr g on g.id_group = tov.Group_raspr and r2.tt_format_rasp = g.tt_format_group

    left join #rasp_gr_tt rgt on rgt.id_tt = r.id_tt and rgt.id_group_rasp = tov.id_group
    ...
    and not ((tov.id_group_init in (65, 10174, 10176) and tov.all_nesklad=0) and r2.koef_ost_pr_rasp is not null)  -- товар из ФРОВ кроме борщевого набора без Избытков
	
	and not (isnull(r2.type_reserve,0) in (1,4)) -- значит это добавлены тт, которые уже отправлены
    ...
    --and a.id_tov = 52  and  a.id_tt = 625
...
----------------------------------------------------------------------------
	...
	(select r.number_r,r.id_tov , r.id_kontr , SUM(r.q_raspr + ISNULL(r.q_raspr_2,0)) q_rasp
	from M2..rasp r with (index (ind1) )
	where r.number_r=@N
...
-- убрать распределения более остатка
create table #tov_kontr_diff (id_tov int , id_kontr int , diff real )
...
truncate table #tov_kontr_diff
insert into #tov_kontr_diff
select a.id_tov , a.id_kontr , a.q_raspr - tk.q_ost_sklad diff
from 
(select r.number_r , r.id_tov , r.id_kontr , sum(q_raspr) q_raspr
from m2..rasp r
where r.number_r=@N
group by r.number_r , r.id_tov , r.id_kontr
having  sum(q_raspr)>0
) a
inner join m2..tov_kontr_init tk on tk.id_tov = a.id_tov and tk.id_kontr = a.id_kontr and tk.Number_r = a.number_r
--inner join m2..Raspr_zadanie rz on rz.Number_r = a.number_r
where a.q_raspr>tk.q_ost_sklad+0.001


update m2..rasp
set q_raspr = r.q_raspr - master.dbo.minz(r.q_raspr , a.diff)
from m2..rasp r
inner join
(
select top 1 r.id_tt , r.id_tov , r.id_kontr  ,tkd.diff   -- r.id_kontr , r.q_raspr , 
--case when r.q_raspr / tk.Kolvo_korob - floor(r.q_raspr / tk.Kolvo_korob)<0.01 then 0 else 1 end
from m2..rasp r
inner join #tov_kontr_diff tkd on r.id_tov = tkd.id_tov and r.id_kontr = tkd.id_kontr
inner join m2..tov_kontr tk on tk.id_tov = r.id_tov and tk.id_kontr = r.id_kontr and tk.Number_r = r.number_r
where r.number_r = @N and q_raspr>0
order by ROW_NUMBER() over (partition by r.id_tov, r.id_kontr order by 
case when r.q_raspr / tk.Kolvo_korob - floor(r.q_raspr / tk.Kolvo_korob)<0.01 then 0 else 1 end desc ,  -- сначала дробные коробки
r.tt_format_rasp , 
r.q_raspr )
) a on a.id_tt = r.id_tt and a.id_tov = r.id_tov and a.id_kontr = r.id_kontr
where r.number_r=@N
...
--FROM #1C_add
	...
	--				  id,
			  ...
			  END
			  INSERT INTO jobs..error_jobs (job_name , message , number_step , id_job)
			  SELECT @Job_name, @err_str, 1691, @id_job		  
...
select @getdate = getdate() 
   ...
   if @type_raspr = 1
   begin
   -- поставить минуса на rasp и raspr_hystory
   ...
   where number_r = @N
	...
	begin 
		...
		exec m2..send_email_job @id_job, @N
...
--declare @N int = 84853
   ...
   Declare @ТекстИДТоваров nvarchar(max) 
     ...
     where t2.Number_r is null and @type_raspr=0
     ) 
...
begin catch
    ...
    if @test_raspr =0
    BEGIN
		exec M2..Notification_Raspr_err @id_job,@err_str,@N
