...
CREATE PROCEDURE [dbo].[Recalc_DTT_add_trigger_20190307]
@id_job	as int
--,@id_tt as int
AS
BEGIN
...
from (select id_tt
		, date_tt
		, sum( post * pr.Price) post_sum
		, sum( digust * pr.Price) degust_sum
		, sum( spisanie* pr.Price) spisanie_sum
		, sum( spisanie_kach * pr.Price) spisanie_kach_sum
		, sum( boi * pr.Price) boi_sum
		, sum( spisanie_dost * pr.Price) spisanie_dost_sum
		, sum( akcia * pr.Price) akcia_sum
		, sum( discount50) discount50_sum
		, sum( razniza * pr.Price)  razniza_sum
		, sum( summa) summa
		, sum( vozvrat_sum)  vozvrat_sum
		, sum( peremPlus * pr.Price) peremPlus_sum
		, sum( peremMinus * pr.Price) peremMinus_sum
		, sum( obed * pr.Price) obed_sum
		, sum( otgruzka_UL_sum) otgruzka_UL_sum
		from #dtt as d 
 ...
 begin try
	insert into [vv03].[dbo].[DTT]
	 ...
	 select d_upd.[date_tt]
           ,isnull(tt_gr.id_gr, tt.id_group)[id_group]
           ,d_upd.[id_tt]
           ,d_upd.[id_tov]
           ,d_upd.[post]
           ,d_upd.[digust]
	 ...
	 from #dtt as d_upd 
		left join vv03..tt_date_id_gr as tt_gr  with(nolock) 
				on tt_gr.date=d_upd.date_tt 
					and tt_gr.id_tt=d_upd.id_tt
		left join vv03..tt as tt with(nolock) 
				on tt.id_TT=d_upd.id_tt
...
begin
 begin try
	INSERT INTO vv03.[dbo].[DT]
           ...
           ,[Otgruzka_UL_sum] )
	 select d_upd.[date_tt]
           ,isnull(isnull(tt_gr.id_gr, tt.id_group),0)[id_group]
           ,d_upd.[id_tt]
...
begin
 begin try
	SET @strТекстSQLЗапроса = '
	 EXEC('' 
		delete from jobs..Recalc_DTT
		from jobs..Recalc_DTT as dtt inner join Temp_tables.dbo.[' + @temp_table + '] i on dtt.id=i.id 
		 '') at [SRV-SQL01]	' 
	exec sp_executeSQL @strТекстSQLЗапроса		  
    BREAK
