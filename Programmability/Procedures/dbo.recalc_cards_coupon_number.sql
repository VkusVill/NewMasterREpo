...
-- Create date: 
-- Description:	агрегирование данных по карте для вывода на кассу
-- OD VKUS-191 2019-12-05  ЛП установленный на 1 день [vv03].[dbo].[lct_one_day]
-- =============================================
CREATE PROCEDURE [dbo].[recalc_cards_coupon_number] 
@nvaCardNum char(7) 
...
begin
	begin try
			...
			-- ЛП на один день
			SELECT lct.number , lct.id_tov , 0 , 0, null , null, null,  lct.sp_price_one_day,0 , cast(lct.date_add as date), null , lct.proc_sk_one_day
			FROM [vv03].[dbo].[lct_one_day] as lct WITH(NOLOCK)
			WHERE lct.date_add >= CONVERT(date,getdate()) 
						) a
