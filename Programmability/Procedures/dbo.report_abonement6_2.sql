...
-- =============================================
create PROCEDURE [dbo].[report_abonement6_2]
...
if OBJECT_ID ('Reportsreport_abonement6_2') is not null drop table Reportsreport_abonement6_2

select ct.date_activation, ct.par1 , COUNT(distinct ct.number ) ВМагНажали , count(distinct chl.BonusCard) [ВМагКупили], 
convert(int,100.0* count(distinct chl.BonusCard) / COUNT(distinct ct.number )) [%КупВМаг]
, ROW_NUMBER() over (order by ct.date_activation, ct.par1) rn
into Reportsreport_abonement6_2
...
and ct.date_activation >= dateadd(day,-3,CONVERT(date,getdate()))
and ct.par3 is null
group by ct.date_activation, ct.par1
